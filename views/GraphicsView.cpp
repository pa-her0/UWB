...
void GraphicsView::scaleView(qreal sx, qreal sy, QPointF center)
{
    ...
    visibleRect.setWidth(qBound(1., visibleRect.width() * sx, 5000.));
    visibleRect.setHeight(qBound(1., visibleRect.height() * sy, 5000.));
    visibleRect.translate(-diff.x() * visibleRect.width() / this->_visibleRect.width(),
                          -diff.y() * visibleRect.height() / this->_visibleRect.height());

    qreal scale = qBound(1., visibleRect.height() * sy, 5000.);
    currentScale = scale;
...
void GraphicsView::drawGrid(QPainter *painter, const QRectF &rect)
{
    ...
    double max = std::max(width,height)/0.5;

    QRectF adjusted = rect.adjusted(-0.025*max, -0.025*max, 0.025*max, 0.025*max);

    double left = width * qCeil(adjusted.left() / width);
    double top = height * qCeil(adjusted.top() / height);

    painter->setPen(QPen(QBrush(Qt::lightGray), 0.0));
    ...
    if (width > 0 && height > 0)
    {
        for (double x = left; x < adjusted.right(); x+= width)
        {
            ...
            {
                painter->drawLine(QPointF(x, y-0.025*max), QPointF(x, y+0.025*max));
                painter->drawLine(QPointF(x-0.025*max, y), QPointF(x+0.025*max, y));
            }
        }
    }
}
