...
RTLSDisplayApplication::RTLSDisplayApplication(int &argc, char **argv) : QApplication(argc, argv)
{
    ...
    QObject::connect(_client, SIGNAL(tagRange(quint64,quint64,double,double,int)), graphicsWidget(), SLOT(tagRange(quint64,quint64,double,double,int)));
...
UwbDataVizWidget *RTLSDisplayApplication::uwbDataVizWidget()
{
    return mainWindow()->uwbDataVizWidget();
}

InfluxWriteService *RTLSDisplayApplication::influxWriteService()
{
    RTLSDisplayApplication *app = instance();
    if (!app->_influxWriteService && app->uwbDataVizWidget()) {
